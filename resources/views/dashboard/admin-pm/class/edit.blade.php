        ...
        <div class="flex w-full flex-col">
            <div class="py-4 md:pb-7 md:pt-12">
                <div class="mb-2 flex items-center gap-2">
                    ...
                    <p class="text-base font-bold leading-normal text-gray-800 sm:text-lg md:text-2xl lg:text-4xl">Edit
                        Daftar Class</p>
                </div>
                <p class="w-full text-disabled">Formulir untuk mengubah data kelas, beserta thumbnail, deskripsi, dan silabus
                    dinamis.</p>
            </div>
            <div class="rounded-3xl bg-white px-4 py-4 shadow-lg shadow-gray-400 md:px-8 md:py-7 xl:px-10">
                <form action="{{ route('dashboard.admin.class.update', $class->id) }}" method="POST"
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <div>
                            ...
                            <div class="relative w-full mt-2">
                                <textarea id="description" name="description" placeholder="Deskripsi singkat mengenai kelas ini" rows="4"
                                    class="peer w-full rounded-lg border-2 border-gray-300 px-4 py-4 shadow-sm resize-none" required>{{ old('description', $class->description) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative w-full mt-2 flex flex-col gap-2">
                                @if ($class->thumbnail)
                                    <img src="{{ asset('uploads/thumbnails/' . $class->thumbnail) }}" alt="Thumbnail"
                                        class="w-40 rounded-lg shadow mb-2">
                                @endif
                                ...
                                <small class="text-gray-500">Kosongkan jika tidak ingin mengganti thumbnail.</small>
                            </div>
                        </div>
...
@section('script')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Data silabus dari PHP
            let syllabusData = @json(json_decode($class->syllabus, true) ?? []);

            function addSyllabusItem(title = '', details = '') {
                ...
                <div class="flex flex-col gap-2">
                    <div>
                        ...
                        <input type="text" name="syllabus[${syllabusIndex}][title]" id="syllabus_${syllabusIndex}_title" placeholder="Contoh: Pengantar Sumber Daya Manusia" class="w-full rounded-lg border-2 border-gray-300 px-3 py-2 mt-1" value="${title}" required>
                    </div>
                    <div>
                        <label for="syllabus_${syllabusIndex}_details" class="text-sm font-medium text-gray-700">Detail Sub Bab</label>
                        <textarea name="syllabus[${syllabusIndex}][details]" id="syllabus_${syllabusIndex}_details" placeholder="Jelaskan detail dari bab ini" rows="3" class="w-full rounded-lg border-2 border-gray-300 px-3 py-2 mt-1 resize-none" required>${details}</textarea>
                    </div>
                </div>
