                                        ...
                                        <tr class="border-b border-gray-200 hover:bg-gray-200 odd:bg-gray-50 even:bg-white">
                                            ...
                                            <td class="whitespace-no-wrap flex items-center justify-center gap-2 px-6 py-4">
                                                <a href="javascript:void(0);"
                                                    onclick="showQuestionDetail({{ $class->id }}, {{ $pretestId }}, {{ $question->id }})"
                                                ...
                                                <a href="javascript:void(0);"
                                                    onclick="editQuestionDetail({{ $class->id }}, {{ $pretestId }}, {{ $question->id }})"
                                                ...
                                                <button type="button"
                                                    onclick="openDeleteModal({{ $class->id }}, {{ $pretestId }}, {{ $question->id }})"
    ...
    <div id="createQuestionModal" class="fixed inset-0 z-10 flex hidden items-center justify-center bg-black bg-opacity-50">
        <div class="relative w-full max-w-xl rounded-xl bg-white p-6 text-center">
            ...
            <form id="createSoalForm" method="POST"
                action="{{ route('dashboard.kelas.pre-test.store', ['class' => $class->id, 'pretest' => $pretestId]) }}">
                @csrf
                <input type="hidden" name="test_id" value="{{ $pretestId }}">
                @csrf
            ...
            <div class="mb-5 mt-6 flex flex-col gap-4">
                ...
                <div class="text-left">
                    ...
                    <input type="text" id="detail_question"
                        class="w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2" value="" readonly />
                </div>
...
@section('script')
    <script>
        ...
        function openDeleteModal(classId, pretestId, questionId) {
            deleteForm.action = `/dashboard/kelas/${classId}/soal-pre-test/${pretestId}/delete/${questionId}`;
    ...
    <script>
        ...
        function editQuestionDetail(classId, pretestId, questionId) {
            fetch(`/dashboard/kelas/${classId}/soal-pre-test/${pretestId}/show/${questionId}`)
                .then(response => response.json())
                .then(data => {
                    openEditQuestionModal(data, classId, pretestId);
                })
                .catch(error => {
        ...
        function openEditQuestionModal(soal, classId, pretestId) {
            document.getElementById('editQuestionModal').classList.remove('hidden');

            // Set form action URL untuk update
            const updateUrl = `/dashboard/kelas/${classId}/soal-pre-test/${pretestId}/update/${soal.id}`;
    ...
    <script>
        function showQuestionDetail(classId, pretestId, questionId) {
        ...
        function openDetailSoalModal(soal) {
            ...
            if (soal.type === 'multiple_choice' && soal.options) {
                ...
                optionsArray.forEach(function(option) {
                    ...
                    pilihanDiv.innerHTML = `
                        <div class="font-bold w-1/12 h-auto py-2 bg-primary rounded-lg text-white text-center">${abjad}.</div>
                        <input type="text" class="w-2/3 rounded-lg border border-gray-300 bg-gray-100 px-3 py-2" value="${jawaban}" readonly>
