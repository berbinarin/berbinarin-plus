...
@section('content')
                    ...
                    <div>
                        <div class="mb-2 flex items-center gap-2">
                            ...
                            <p class="text-base font-bold leading-normal text-gray-800 sm:text-lg md:text-2xl lg:text-4xl">
                                Daftar Soal Post Test</p>
                        </div>
                        <p class="w-full text-disabled">
                            Halaman untuk menambahkan atau mengubah soal baru ke dalam program Berbinar+ secara rinci,
                            beserta detail soal, dan jawaban.
                        </p>
                        <a href="javascript:void(0);" onclick="openCreateQuestionModal()"
                            ...
                            <table id="example" class="display min-w-full pt-5 leading-normal">
                                <thead>
                                    <tr class="mt-4">
                                        ...
                                        <th
                                            class="w-4/12 sticky-col sticky-col-2 bg-white px-6 py-3 text-start text-base font-bold leading-4 tracking-wider text-black">
                                            Tipe Soal
                                        </th>
                                        <th
                                            class="w-4/12 sticky-col sticky-col-2 bg-white px-6 py-3 text-start text-base font-bold leading-4 tracking-wider text-black">
                                    ...
                                    @foreach ($questions as $index => $question)
                                        <tr class="border-b border-gray-200 hover:bg-gray-200 odd:bg-gray-50 even:bg-white">
                                            ...
                                            <td class="whitespace-no-wrap sticky-col sticky-col-2 px-6 py-4">
                                                {{ $question->type === 'multiple_choice' ? 'Pilihan Ganda' : 'Jawaban Singkat' }}
                                            </td>
                                            <td class="whitespace-no-wrap sticky-col sticky-col-2 px-6 py-4">
                                            ...
                                            <td class="whitespace-no-wrap flex items-center justify-center gap-2 px-6 py-4">
                                                <a href="javascript:void(0);"
                                                    onclick="showQuestionDetail({{ $class->id }}, {{ $posttestId }}, {{ $question->id }})"
                                                ...
                                                <a href="javascript:void(0);"
                                                    onclick="editQuestionDetail({{ $class->id }}, {{ $posttestId }}, {{ $question->id }})"
                                                ...
                                                <button type="button"
                                                    onclick="openDeleteModal({{ $class->id }}, {{ $posttestId }}, {{ $question->id }})"
    ...
    <div id="createQuestionModal" class="fixed inset-0 z-10 flex hidden items-center justify-center bg-black bg-opacity-50">
        <div class="relative w-full max-w-xl rounded-xl bg-white p-6 text-center">
            ...
            <form id="createSoalForm" method="POST"
                action="{{ route('dashboard.kelas.post-test.store', ['class' => $class->id, 'posttest' => $posttestId]) }}">
                @csrf
                <input type="hidden" name="test_id" value="{{ $posttestId }}">
                @csrf
                <div class="mb-5 mt-6 flex flex-col gap-4">
                    <div class="text-left">
                        <label class="mb-1 block font-medium text-gray-600">Tipe Soal</label>
                        <select id="type" name="type" class="w-full rounded-lg border border-gray-300 px-3 py-2"
                            required>
                    ...
                    <div class="text-left">
                        ...
                        <input type="text" id="question" name="question"
                            ...
                            placeholder="Berapa hasil dari 2 + 5?" required />
                    </div>
                    <div id="multipleChoiceSection" class="max-h-52 overflow-y-auto">
                        <div class="text-left">
                            <div class="mb-6 mt-2 flex cursor-pointer items-center justify-center rounded-lg border border-dashed border-blue-500 py-2 text-blue-500"
                                id="addOptionButton">
                    ...
                    <div id="shortAnswerSection" class="hidden max-h-52 overflow-y-auto">
                        ...
                        <input type="text" name="correct_answer"
                            class="w-full rounded-lg border border-gray-300 px-3 py-2" placeholder="Jawaban uraian..." />
                    </div>
                </div>
                <div class="flex w-full justify-center gap-4">
                    <button type="button"
                        ...
                        onclick="closeCreateQuestionModal()">Batal</button>
                    <button type="submit"
        ...
        <div class="relative w-full max-w-xl rounded-xl bg-white p-6 text-center">
            ...
            <form id="editSoalForm" method="POST" action="">
                ...
                <div class="mb-5 mt-6 flex flex-col gap-4">
                    <div class="text-left">
                        <label class="mb-1 block font-medium text-gray-600">Tipe Soal</label>
                        <select id="edit_type" name="type" class="w-full rounded-lg border border-gray-300 px-3 py-2"
                            required>
                            <option value="multiple_choice">Pilihan Ganda</option>
                            <option value="short_answer">Jawaban Singkat</option>
                        </select>
                ...
                <div class="flex w-full justify-center gap-4">
                    <button type="button"
                        class="w-1/2 rounded-lg border border-[#3986A3] px-6 py-2 text-[#3986A3] focus:outline-none focus:ring-2 focus:ring-[#3986A3] focus:ring-offset-2"
                        onclick="closeEditQuestionModal()">Batal</button>
                    <button type="submit"
                        class="w-1/2 rounded-lg bg-[#3986A3] px-6 py-2 text-center text-white hover:bg-[#3986A3] focus:outline-none focus:ring-2 focus:ring-[#3986A3] focus:ring-offset-2">Simpan</button>
                </div>
            </form>
    ...
    <div id="detailQuestionModal"
        ...
        <div class="relative w-full max-w-xl rounded-xl bg-white p-6 text-center">
            <h3 class="mb-4 text-xl font-bold leading-6 text-black">
                Detail Soal Post Test
            </h3>
            <div class="mb-5 mt-6 flex flex-col gap-4">
                <div class="text-left">
                    <label class="mb-1 block font-medium text-gray-600">Tipe Soal</label>
                    <select id="detail_type" class="w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2"
                        disabled>
                        <option value="multiple_choice">Pilihan Ganda</option>
                        <option value="short_answer">Jawaban Singkat</option>
                    </select>
                ...
                <div class="text-left">
                    <label class="mb-1 block font-medium text-gray-600">Pertanyaan</label>
                    <input type="text" id="detail_question"
                        class="w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2" readonly />
                </div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex hidden items-center justify-center bg-black/40">
        <div class="relative w-[360px] md:w-[560px] rounded-[20px] bg-white p-6 text-center font-plusJakartaSans shadow-lg"
            ...
            <!-- Message -->
            <p class="mt-2 text-base font-medium text-black">Apakah Anda yakin ingin menghapus soal ini? Data soal
...
@section('script')
    <script>
        ...
        function openDeleteModal(classId, posttestId, questionId) {
            deleteForm.action = `/dashboard/kelas/${classId}/soal-post-test/${posttestId}/delete/${questionId}`;
    ...
    <script>
        ...
        // Fetch data soal dan buka modal edit
        function editQuestionDetail(classId, posttestId, questionId) {
            fetch(`/dashboard/kelas/${classId}/soal-post-test/${posttestId}/show/${questionId}`)
                .then(response => response.json())
                .then(data => {
                    openEditQuestionModal(data, classId, posttestId);
                })
                .catch(error => {
        ...
        function openEditQuestionModal(soal, classId, posttestId) {
            document.getElementById('editQuestionModal').classList.remove('hidden');

            // Set form action URL untuk update
            const updateUrl = `/dashboard/kelas/${classId}/soal-post-test/${posttestId}/update/${soal.id}`;
            document.getElementById('editSoalForm').action = updateUrl;

            document.getElementById('edit_type').value = soal.type;
            document.getElementById('edit_question').value = soal.question_text;

            // Tampilkan/hide section sesuai type
            if (soal.type === 'short_answer') {
                editPilihanGandaSection.classList.add('hidden');
                editUraianSection.classList.remove('hidden');
                // Isi jawaban short_answer
                editUraianSection.querySelector('input[name="correct_answer"]').value = soal.correct_answer || '';
            } else {
                editPilihanGandaSection.classList.remove('hidden');
                editUraianSection.classList.add('hidden');
            }
            ...
            if (soal.type === 'multiple_choice' && soal.options) {
                // Konversi object options ke array jika perlu
                const optionsArray = Array.isArray(soal.options) ? soal.options : Object.entries(soal.options).map(([key,
                    value
                ]) => ({
                    key: key,
                ...
                optionsArray.forEach(function(option) {
                    const abjad = option.key;
                    const jawaban = option.jawaban;
                    const status = abjad === soal.correct_answer ? 'benar' : 'salah';
                    ...
                    pilihanDiv.innerHTML = `
                        <div class="font-bold w-1/12 h-auto py-2 bg-primary rounded-lg text-white text-center">${abjad}.</div>
                        <input type="text" name="options[${abjad}][jawaban]" class="w-2/3 rounded-lg border border-gray-300 px-3 py-2" value="${jawaban}" required>
                        <select name="options[${abjad}][status]" class="rounded-lg appearance-none bg-none border border-gray-300 px-2 py-2">
    ...
    <script>
        function showQuestionDetail(classId, posttestId, questionId) {
            // Fetch question data from server
            fetch(`/dashboard/kelas/${classId}/soal-post-test/${posttestId}/show/${questionId}`)
                .then(response => response.json())
                .then(data => {
                    openDetailSoalModal(data);
                })
                .catch(error => {
                    console.error('Error fetching question:', error);
        ...
        function openDetailSoalModal(soal) {
            ...
            typeSelect.value = soal.type;

            const questionInput = document.getElementById('detail_question');
            questionInput.value = soal.question_text;
            ...
            if (soal.type === 'multiple_choice' && soal.options) {
                ...
                optionsArray.forEach(function(option) {
                    const abjad = option.key;
                    const jawaban = option.jawaban;
                    const isCorrect = abjad === soal.correct_answer;
                    const statusClass = isCorrect ? 'bg-green-500 text-white' : 'bg-red-500 text-white';
                    ...
                    pilihanDiv.innerHTML = `
                        <div class="font-bold w-1/12 h-auto py-2 bg-primary rounded-lg text-white text-center">${abjad}.</div>
                        <input type="text" class="w-3/5 rounded-lg border border-gray-300 px-3 py-2 bg-gray-100" value="${jawaban}" readonly>
                        <div class="w-1/4 rounded-lg px-3 py-2 text-center font-medium ${statusClass}">${statusText}</div>
                    `;
                    pilihanContainer.appendChild(pilihanDiv);
                });
            } else if (soal.type === 'short_answer') {
                shortAnswerContainer.style.display = 'block';
                shortAnswerText.textContent = soal.correct_answer || '-';
            }
        }
    </script>
@endsection
