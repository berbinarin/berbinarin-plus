...
@section('content')
    <div class="mt-20 lg:mt-10">
        <div class="w-full flex flex-col">
            ...
            <nav class="text-gray-500 max-sm:text-sm text-lg" aria-label="Breadcrumb">
                ...
                <span>/</span>
                <span class="text-black">Post Test - {{ $class->name ?? '-' }}</span>
            </nav>

            <h1 class="mt-4 lg:mt-6 text-xl lg:text-3xl font-semibold">Post Test - {{ $class->name ?? '-' }}</h1>

            <div class="bg-white w-full mt-5 lg:mt-10 rounded-2xl p-5 lg:p-8">
                <h2 class="mb-6 font-medium text-lg lg:text-xl">
                    {{ $posttest && $posttest->questions ? $posttest->questions->count() : 0 }} Soal Terlampir
                </h2>
                <button type="button" id="showModal"
                    class="bg-primary text-white py-1 px-4 lg:py-1 rounded-lg text-lg lg:text-xl gap-2">Mulai Post
                    Test</button>
            </div>
        </div>
    </div>
    ...
    <div id="confirmModal" class="fixed inset-0 z-50 flex hidden items-center justify-center bg-black/40">
        <div class="relative w-[90%] lg:w-[560px] rounded-[20px] bg-white p-3 lg:p-6 text-center font-plusJakartaSans shadow-lg"
            ...
            <!-- Message -->
            <p class="mt-1 lg:mt-2 text-sm lg:text-base font-medium text-black">Siap mulai? Pre test ini akan mengukur
                pemahaman awalmu sebelum lanjut ke materi.</p>
            ...
            <div class="mt-3 lg:mt-6 flex justify-center gap-3">
                ...
                <a href="{{ route('landing.posttest.question', ['class_id' => $class->id ?? 1, 'number' => 1]) }}"
...
@section('script')
    <script>
        ...
        // Hapus jawaban posttest di localStorage saat halaman index posttest dibuka
        document.addEventListener("DOMContentLoaded", function() {
