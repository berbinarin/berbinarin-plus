                ...
                <div class="py-4 md:pt-12 md:pb-7">
                    <div>
                        <p tabindex="0"
                            ...
                            Paket <span class="">Layanan</span>
                        </p>
                        <p class="w-full text-[#333333] lg:text-lg">
                            Halaman ini berisi daftar layanan kelas yang tersedia dalam program Berbinar+.
                        ...
                        <a href="{{ route('dashboard.admin.layanan.create') }}"
                                ...
                                <thead>
                                    <tr class="mt-4">
                                        ...
                                        <th
                                            ...
                                            Jadwal</th>
                                        <th
                                            class="bg-white px-6 py-3 text-start text-base font-bold leading-4 tracking-wider text-black">
                                            Description</th>
                                        <th
                                            class="bg-white px-6 py-3 text-start text-base font-bold leading-4 tracking-wider text-black">
                                            Harga</th>
                                    ...
                                    @foreach ($services as $i => $service)
                                        <tr
                                            ...
                                            <td class="whitespace-no-wrap sticky-col sticky-col-2 px-6 py-4">
                                                {{ $service->name }}</td>
                                            <td class="whitespace-no-wrap px-6 py-4">{{ $service->type }}</td>
                                            <td class="whitespace-no-wrap px-6 py-4">{{ $service->schedule }}</td>
                                            <td class="whitespace-no-wrap px-6 py-4">{{ $service->description }}</td>
                                            ...
                                            <td class="whitespace-no-wrap flex items-center justify-center gap-2 px-6 py-4">
                                                <a href="{{ route('dashboard.admin.layanan.show', $service->id) }}"
                                                ...
                                                <a href="{{ route('dashboard.admin.layanan.edit', $service->id) }}"
    ...
    <div id="deleteModal" class="fixed inset-0 z-10 hidden items-center justify-center bg-black bg-opacity-50 flex">
        <div class="w-full max-w-md rounded-lg bg-white p-6 text-center">
            <div class="mb-4 flex justify-center">
                <img src="{{ asset('assets/images/dashboard/svg/warning.svg') }}" alt="Warning Icon" class="h-12 w-12" />
            </div>
            <h3 class="mb-2 text-lg font-medium leading-6 text-gray-900" id="modal-title">Konfirmasi Hapus</h3>
            <p class="mb-6 text-base text-gray-500">Apakah Anda yakin ingin menghapus kelas ini? Semua data terkait juga
                akan dihapus.</p>
            <div class="flex w-full justify-center gap-4">
                <form id="deleteForm" method="POST" class="w-1/2">
                    ...
                    @method('DELETE')
                    <button type="submit"
                        class="rounded-lg bg-[#3986A3] w-full px-6 py-2 text-white text-center hover:bg-[#3986A3] focus:outline-none focus:ring-2 focus:ring-[#3986A3] focus:ring-offset-2">Hapus</button>
                </form>
                <button type="button"
                    class="rounded-lg border border-[#3986A3] w-1/2 px-6 py-2 text-[#3986A3] focus:outline-none focus:ring-2 focus:ring-[#3986A3] focus:ring-offset-2"
...
@section('script')
    <script>
        ...
        function openDeleteModal(serviceId) {
            deleteForm.action = `/dashboard/admin/layanan/${serviceId}`;
