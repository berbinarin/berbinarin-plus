    ...
    <div class="flex w-full flex-col">
        <div class="py-4 md:pb-7 md:pt-12">
            <div class="mb-2 flex items-center gap-2">
                ...
                <p class="text-base font-bold leading-normal text-gray-800 sm:text-lg md:text-2xl lg:text-4xl">Edit Paket Layanan</p>
            </div>
            <p class="w-full text-disabled lg:text-lg">Formulir untuk mengubah data paket layanan yang ditawarkan di semua kelas.</p>
        </div>
        <div class="rounded-3xl bg-white px-4 py-4 shadow-lg shadow-gray-400 md:px-8 md:py-7 xl:px-10">
            
            <form action="{{ route('dashboard.admin.layanan.update', $service->id) }}" method="POST">
                @csrf
                @method('PUT')

                <h1 class="mb-6 text-center text-3xl font-bold">Edit Paket Layanan</h1>
                    ...
                    <div>
                        ...
                        <div class="relative w-full mt-2">
                            <select id="type" name="type"
                                ...
                                <option value="Offline" {{ old('type', $service->type) == 'Offline' ? 'selected' : '' }}>Offline</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative w-full mt-2">
                            <select id="schedule" name="schedule"
                                ...
                                <option value="Weekday" {{ old('schedule', $service->schedule) == 'Weekday' ? 'selected' : '' }}>Weekday</option>
                                <option value="Weekend" {{ old('schedule', $service->schedule) == 'Weekend' ? 'selected' : '' }}>Weekend</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative w-full mt-2">
                            <textarea id="description" name="description" placeholder="Deskripsi singkat fitur-fitur paket ini" rows="4"
                                class="peer w-full rounded-lg border-2 border-gray-300 px-4 py-4 shadow-sm resize-none">{{ old('description', $service->description) }}</textarea>
                        </div>
                    </div>
