...
@section('content')
        ...
        <div class="w-full flex flex-col">
            ...
            <div class="bg-white w-full mt-5 lg:mt-10 rounded-2xl p-5 lg:p-8">
                <h2 class="mb-6 font-medium text-lg lg:text-xl">
                    {{ $pretest && $pretest->questions ? $pretest->questions->count() : 0 }} Soal Terlampir</h2>

                <button type="button" id="showModal"
                    class="bg-primary text-white py-1 px-4 lg:py-1 rounded-lg text-lg lg:text-xl gap-2">Mulai Pre
                    Test</button>
            </div>
        </div>
    </div>
    ...
    <div id="confirmModal" class="fixed inset-0 z-50 flex hidden items-center justify-center bg-black/40">
        <div class="relative w-[90%] lg:w-[560px] rounded-[20px] bg-white p-3 lg:p-6 text-center font-plusJakartaSans shadow-lg"
            ...
            <!-- Title -->
            <h2 class="mt-2 lg:mt-4 text-lg lg:text-2xl font-bold text-stone-900">Mulai Pre Test!</h2>
            ...
            <div class="mt-3 lg:mt-6 flex justify-center gap-3">
                ...
                <a href="{{ route('landing.pretest.question', ['class_id' => $class->id ?? 1, 'number' => 1]) }}"
