        ...
        <div class="flex flex-col w-full bg-gray-100">
            <div class="w-full">
                <div class="py-4 md:pt-12 md:pb-7">
                    <div>
                        <div class="mb-2 flex items-center gap-2">
                            <a
                                href="{{ route('dashboard.pendaftar.pengumpulan-tes.index', ['user' => $userModel->id ?? null, 'enrollment' => $enrollmentModel->id ?? null]) }}">
                            ...
                            <p class="text-base font-bold leading-normal text-gray-800 sm:text-lg md:text-2xl lg:text-4xl">
                                Detail Jawaban Post Test</p>
                        </div>
                        <p class="w-full text-disabled">
                            Halaman yang menampilkan detail jawaban peserta pada Post Test, lengkap dengan hasil penilaian
                            dan data peserta untuk memudahkan proses evaluasi.
                        </p>
                    </div>
                </div>
                <div class="w-full space-x-4 flex flex-row gap-3">
                    ...
                    <div class="flex flex-col gap-5 w-1/3">
                        ...
                        <div class="bg-[#106681] text-white flex flex-col gap-5 justify-between px-6 py-4 rounded-xl">
                            <div class="flex flex-col gap-1">
                                ...
                                <h2 class="text-xl font-medium">
                                    {{ isset($benar) ? $benar : '-' }} Soal
                                </h2>
                            </div>
                            <div class="flex flex-col gap-1">
                                <h1>salah</h1>
                                <h2 class="text-xl font-medium">
                                    {{ isset($salah) ? $salah : '-' }} Soal
                                </h2>
                            </div>
                        </div>
                        ...
                        <div class="bg-white flex flex-col gap-5 justify-between px-6 py-4 rounded-xl">
                            <div class="flex flex-col gap-1">
                                ...
                                <h2 class="text-xl font-medium">
                                    @if (isset($userModel))
                                        {{ trim(($userModel->first_name ?? '') . ' ' . ($userModel->last_name ?? '')) ?: '-' }}
                                    @else
                                        -
                                    @endif
                                </h2>
                            ...
                            <div class="flex flex-col gap-1">
                                <h1>Email</h1>
                                <h2 class="text-xl font-medium">{{ $userModel->email ?? '-' }}</h2>
                            </div>
                            <div class="flex flex-col gap-1">
                                <h1>Tanggal Pengerjaan</h1>
                                <h2 class="text-xl font-medium">{{ $tanggalPengerjaan ?? '-' }}</h2>
                            </div>
                            <div class="flex flex-col gap-1">
                                <h1>status</h1>
                    ...
                    <div class="w-2/3">
                        <div
                            class="bg-white flex flex-col gap-5 justify-between px-6 py-4 rounded-xl overflow-y-auto max-h-[72vh]">
                            ...
                            <div class="flex flex-col overflow-y-auto">
                                <table class="w-full h-full gap-3 mb-6" style="overflow-y: scroll">
                                    ...
                                    <tbody class="px-2">
                                        @if (isset($jawabanList) && count($jawabanList) > 0)
                                            @foreach ($jawabanList as $i => $jawaban)
                                                <tr>
                                                    <td class="py-4 text-center">{{ $i + 1 }}</td>
                                                    <td class="text-start">{{ $jawaban['pertanyaan'] }}</td>
                                        ...
                                        @else
                                            <tr>
                                                <td colspan="4" class="text-center text-gray-400">Tidak ada data jawaban
                                                </td>
                                            </tr>
                                        @endif
